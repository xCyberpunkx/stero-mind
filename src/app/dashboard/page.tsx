...
import { NeuroLogsSection } from '@/components/NeuroLogsSection'
...
export default async function DashboardPage() {
    ...
    const widgets = [
      { title: "Projects", icon: Briefcase, count: projectCount || "0", status: "ACTIVE" },
      { title: "Sessions", icon: Clock, count: sessionCount || "0", status: "SYNCED" },
      { title: "Tasks", icon: Brain, count: taskCount || "0", status: "QUEUE" },
      { title: "Stats", icon: LineChart, count: "ALPHA", status: "LIVE" },
    ]

    return (
        <div className="min-h-screen bg-background text-foreground font-display selection:bg-black selection:text-white pb-24">
          ...
          <main className="relative pt-32 px-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
              ...
              <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <NeuroLogsSection userId={user.id} />
              </div>
            </div>
          </main>
        </div>
