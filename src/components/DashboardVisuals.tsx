        ...
        <div className="space-y-8">
            ...
            <div className="border-2 border-black bg-white p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="font-bold uppercase tracking-tighter font-code">Temporal_Map</h3>
                </div>
                <div className="grid grid-cols-7 gap-1">
                    {[...Array(28)].map((_, i) => {
                        const hasActivity = logs.some(log => {
                            const logDate = new Date(log.created_at).getDate()
                            return logDate === i + 1
                        })
                        return (
                            <div
                                key={i}
                                className={`aspect-square border-2 border-black flex items-center justify-center text-[8px] font-bold font-code transition-colors
                  ${hasActivity ? 'bg-secondary' : 'bg-white'} 
                  ${new Date().getDate() === i + 1 ? 'bg-black text-white' : ''}
                `}
                            >
                                {i + 1}
                            </div>
                ...
                <div className="mt-4 flex justify-between items-center font-code text-[9px] font-bold opacity-50 uppercase">
                    <div className="flex items-center gap-1"><div className="w-2 h-2 bg-secondary border border-black" /> Activity</div>
                    <div className="flex items-center gap-1"><div className="w-2 h-2 bg-black border border-black" /> Today</div>
                </div>
            </div>
        </div>
    )
