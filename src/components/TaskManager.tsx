        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                ...
                {initialTasks.map((task) => (
                    <div key={task.id} className={`border-2 border-black p-4 bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-between group transition-all ${task.completed ? 'opacity-50' : ''}`}>
                        <div className="flex items-center gap-4">
                            ...
                            <span className={`font-bold uppercase tracking-tight text-sm ${task.completed ? 'line-through' : ''}`}>
                                {task.title}
                            </span>
                        </div>
                        <button
